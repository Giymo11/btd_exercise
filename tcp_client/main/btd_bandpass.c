...
// this is based on https://github.com/curiores/ArduinoTutorials/blob/main/BasicFilters/ArduinoImplementations/LowPass/LowPass2.0/LowPass2.0.ino
...
float apply_filter(BandPassFilter *filter, float raw_value)
{
    ...
    filter->y[0] = filter->b[0] * filter->x[0] + filter->b[1] * filter->x[1] + filter->b[2] * filter->x[2] + filter->a[0] * filter->y[1] + filter->a[1] * filter->y[2];
...
void init_highpass(BandPassFilter *filter, float cutoff_frequency, float sample_frequency)
{
    ...
    // Calculate coefficients
    float alpha = filter->omega0 * filter->dt;
    float dtSq = filter->dt * filter->dt;
    float c[3] = {filter->omega0 * filter->omega0, sqrt(2) * filter->omega0, 1};
    float D = c[0] * dtSq + 2 * c[1] * filter->dt + 4 * c[2];
    ...
    // Feedback coefficients
    filter->a[0] = -(2 * c[0] * dtSq - 8 * c[2]) / D;
    filter->a[1] = -(c[0] * dtSq - 2 * c[1] * filter->dt + 4 * c[2]) / D;
}
